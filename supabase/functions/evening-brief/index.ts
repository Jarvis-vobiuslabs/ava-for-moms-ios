...
async function generateEveningBrief(name: string, openCount: number): Promise<string> {
  const situation = openCount > 0
    ? `She still has ${openCount} unchecked task(s) on her list. Gently nudge her to knock one off or move it to tomorrow — no guilt.`
    : `Her task list is all done. Congratulate her warmly on getting through the day.`
  const res = await anthropic.messages.create({
    model: "claude-haiku-4-5",
    max_tokens: 100,
    messages: [{
      role: "user",
      content: `Write a warm, friendly evening wind-down notification for ${name}, a busy mom.
${situation}
Keep it under 100 characters, like a supportive best friend texting at night.
No punctuation at the end. Just the message text, nothing else.`,
    }],
  })
  return res.content[0].type === "text" ? res.content[0].text.trim() : `Good evening ${name}! Time to wind down`
}
  ...
  try {
    ...
    for (const { user_id, token } of tokens) {
      try {
        ...
        // Runs hourly — only notify users for whom it's currently 8pm
        ...
        const body  = await generateEveningBrief(name, openTasks)
        const title = openTasks > 0
