...
struct AuthView: View {
    ...
    var onboardingData: OnboardingData?   // nil = returning user (sign in only)
    var onComplete: () -> Void
    ...
    init(onboardingData: OnboardingData?, onComplete: @escaping () -> Void) {
        ...
        // New users go to sign up; returning users (no onboarding data) go to sign in
    ...
    var body: some View {
        ZStack {
            ...
            GeometryReader { geo in
                Circle().fill(AvaTheme.blush.opacity(0.2)).frame(width: 200).offset(x: geo.size.width - 60, y: -40)
            }.ignoresSafeArea()
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    // ── Email + password ──────────────────────────────
                    VStack(spacing: 12) {
                        inputField("envelope", placeholder: "Email address",
                    ...
                    Button {
                        ...
                        .padding(.vertical, 18)
                        .background(
                            Capsule().fill(canSubmit ? AnyShapeStyle(AvaTheme.blushTerracotta) : AnyShapeStyle(AvaTheme.line))
                        )
    ...
    // MARK: - After successful auth

    private func finishAuth() async {
        if let data = onboardingData {
            await auth.saveOnboardingData(data)
        }
        onComplete()
    }
    ...
    private func inputField(
        _ icon: String, placeholder: String,
        ...
        HStack(spacing: 12) {
            ...
            .onSubmit {
                ...
                else { Task { if mode == .signUp { await auth.signUp(email: email, password: password) }
